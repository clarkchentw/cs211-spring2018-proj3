...
void setupMaze(int argc, char **argv, maze* mazeBoard){
	...
	// Checking Statement
        while (mazeBoard->xsize <= 0 || mazeBoard->ysize <= 0){
                fprintf(stderr, "Invalid: Maze sizes must be greater than 0\n");
		fscanf (src, "%d %d", &mazeBoard->xsize, &mazeBoard->ysize);
		if (mazeBoard->xsize > 0 && mazeBoard->ysize > 0){
			fprintf(stderr, "Maze become size %d x %d\n", mazeBoard->xsize, mazeBoard->ysize);
		}
        }
	...
	{
		if (mazeBoard->arr[xpos][ypos] == 's' || mazeBoard->arr[xpos][ypos] == 'e'){
...
void displaySolution(maze* mazeBoard, mazeSearch* hd){
	if (hd != NULL){
		if (mazeBoard->arr[hd->x][hd->y] != 's' || mazeBoard->arr[hd->x][hd->y] != 'e')
			mazeBoard->arr[hd->x][hd->y] = '@';
		int i;
		displaySolution(mazeBoard, hd->next);
	}
	else{
		printf("\n\n[------Solution------]\n");
		printMaze(mazeBoard);
...
int main (int argc, char **argv)
{
	...
	if (result == 200){
		displaySolution(&mazeBoard, mazeBoard.hd);
	}
	else if (result == 404){
